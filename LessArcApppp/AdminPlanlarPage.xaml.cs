...
{
    // ▼ API DTO’larına birebir uyum
    public class PlanEkleDto
    {
        public string Baslik { get; set; } = null!;
        public string? Aciklama { get; set; }   // API’de opsiyonel (DB NOT NULL ise "" gönderiyoruz)
        public DateTime Tarih { get; set; }
    }

    public class PlanGuncelleDto
    {
    ...
    public partial class AdminPlanlarPage : ContentPage
    {
        // ========= VERİ =========
        ...
        public double IconSize { get; private set; } = 24;
        ...
        // ========= CTOR =========
        public AdminPlanlarPage(HttpClient httpClient, string kullaniciToken)
        {
            InitializeComponent();

            TemaYonetici.TemayiYukle();

            _http = httpClient;
            token = kullaniciToken ?? string.Empty;

            // Bearer ekle
            if (_http.DefaultRequestHeaders.Authorization is null && !string.IsNullOrWhiteSpace(token))
                _http.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);

            // BaseAddress yoksa göreli URL patlamasın (local → cloud fallback örnek)
            EnsureBaseAddress();
        ...
        private void EnsureBaseAddress()
        {
            ...
            {
                // Buraya kendi backend URL’ini koy: emulator için http://10.0.2.2:7013
                // Masaüstü debug için https://localhost:7013
                // Prod için https://lessarc.com.tr gibi.
        ...
        private void TakvimiOlustur()
        {
            BuildTakvimForGrid(TakvimGrid);
            BuildTakvimForGrid(TakvimGridMobile);
        }

        private void BuildTakvimForGrid(Grid targetGrid)
        {
            ...
            for (int gun = 1; gun <= toplamGun; gun++)
            {
                ...
                var btn = new Button
                {
                    ...
                    Margin = new Thickness(CalendarGridSpacing / 2.0),
        ...
        {
            var entry = DeviceInfo.Idiom == DeviceIdiom.Phone ? entryPlanMobile : entryPlan;
            if (string.IsNullOrWhiteSpace(entry.Text))
            {
                await DisplayAlert("Uyarı", "Plan başlığı boş olamaz.", "Tamam");
                return;
            }

            try
            {
                var dto = new PlanEkleDto
                {
                    Baslik = entry.Text.Trim(),
                    // 🔴 DB'de NOT NULL ihtimaline karşı null değil, boş string gönderiyoruz
                    Aciklama = "",
                    Tarih = secilenTarih.Date
                };

                var response = await _http.PostAsJsonAsync("/api/Planlar", dto);
                if (response.IsSuccessStatusCode)
                {
                    entry.Text = string.Empty;
                    await PlanliGunleriYukleVeTakvimiCiz(); // hem liste hem takvim tazele
                }
                else
                {
                    string body = await response.Content.ReadAsStringAsync();
                    await DisplayAlert("Hata",
                        $"Plan eklenemedi (HTTP {(int)response.StatusCode} {response.ReasonPhrase}):\n{body}",
                        "Tamam");
                }
            }
            catch (Exception ex)
        ...
        private async void PlanSil_Clicked(object sender, EventArgs e)
        {
            ...
            bool onay = await DisplayAlert("Planı Sil",
                $"\"{silinecekPlan.Baslik}\" silinsin mi?", "Evet", "Hayır");
            ...
            try
            {
                ...
                else
                {
                    string body = await response.Content.ReadAsStringAsync();
                    await DisplayAlert("Hata",
                        $"Silinemedi (HTTP {(int)response.StatusCode} {response.ReasonPhrase}):\n{body}",
                        "Tamam");
                }
            }
            catch (Exception ex)
        ...
        private async void PlanDuzenle_Clicked(object sender, EventArgs e)
        {
            ...
            try
            {
                var dto = new PlanGuncelleDto
                {
                    Id = plan.Id,
                    Baslik = yeniBaslik.Trim(),
                    // Aciklama değişmiyorsa mevcut değerini koruyoruz (null olabilir)
                    Aciklama = plan.Aciklama ?? "",
                    Tarih = plan.Tarih
                };

                var response = await _http.PutAsJsonAsync($"/api/Planlar/{plan.Id}", dto);
                if (response.IsSuccessStatusCode)
                {
                    plan.Baslik = dto.Baslik;

                    // UI refresh
                    planListesi.ItemsSource = null;
                    planListesi.ItemsSource = planlar;
                    planListesiMobile.ItemsSource = null;
                    planListesiMobile.ItemsSource = planlar;
                }
                else
                {
                    string body = await response.Content.ReadAsStringAsync();
                    await DisplayAlert("Hata",
                        $"Güncellenemedi (HTTP {(int)response.StatusCode} {response.ReasonPhrase}):\n{body}",
                        "Tamam");
                }
            }
            catch (Exception ex)
